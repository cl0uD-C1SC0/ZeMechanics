...
def adicionar_servico(servico, db):
    ...
    if not servico_adicionado:
        raise HTTPException(status_code=500, detail="Não foi possível adicionar o serviço, tente novamente.")
...
def listar_todos_servicos(db):
    ...
    if not servicos:
        raise HTTPException(status_code=404, detail="Nenhum serviço foi encontrado, tente novamente")
    return servicos
...
def atualizar_servico_especifico(servico_id, dados, db):
    ...
    if not servico_atualizado:
        raise HTTPException(status_code=500, detail="Não foi possível atualizar os dados do serviço, tente novamente")
    return {"message": f"Serviço com o ID: {servico_atualizado.id} atualizado com sucesso."}
...
def remover_servico(servico_id, db):
    ...
    if not servico_removido:
        raise HTTPException(status_code=500, detail="Não foi possível remover o serviço, tente novamente")
