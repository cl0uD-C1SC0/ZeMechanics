...
@router.post("/adicionar_servico", response_model=ServicoResponse)                                   
def cadastrar_servico(servico: ServicoSchema, db: Session = Depends(get_db)):  
...
def consultar_servico(servico_id: int, db: Session = Depends(get_db)):
    servico = servico_service.consultar_servico_especifico(servico_id, db)
    return servico

@router.patch("/atualizar_servico/{servico_id}")
def atualizar_servico(servico_id: int, dados: UpdateServiceSchema, db: Session = Depends(get_db)):
